...
- [x] ハルシネーション対策のギャップを解消: `truncate_runaway_repetition`（直前1行との類似度しか見ていなかった）を数行単位のブロック反復にも対応させようとしたが（`detect_block_repeat_cut`）、実機検証（`reocr_pdf_manual`、`yomitoku_ocr_table_sample_v1.pdf` Page3）で、表の反復には「見出し行の再掲・区切り行の有無による不揃いな間隔」があり、固定長ブロックを固定間隔でスライドさせる方式では検知できないことが判明（誤検知の回帰も2回踏んだ）。表以外での複数行ブロック反復は実例が確認できなかったため、`truncate_runaway_repetition` はブロック拡張前のシンプルな単一行判定に戻し、表の行反復には専用の `find_repeating_rows_cut` を新設して対応することにした: 各行を「それより前に出現した最初に一致する行」に対応付け、対応する過去の行番号が直前行のそれ+1と連続する行が2行続いた時点で反復と判定する。間隔の不揃いに影響されず、`html_table_to_markdown`（`<tr>`パース後の行データ）と `extract_table_markdown` のパイプ行フォールバック（`parse_pipe_row`で行分解）の両方に適用した
  → 検証: `cargo test --lib`（31 passed, 2 ignored、実データを模した回帰テスト2件を含む）。実機再検証（`reocr_pdf_manual`、`yomitoku_ocr_table_sample_v1.pdf` 全5ページ）で、Page3の反復が完全に解消され単一のクリーンな表になることを確認済み
