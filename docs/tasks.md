...
- [x] glm-ocr 単体構成での実機再検証（`1周間SPI_模擬4.pdf`、ユーザーGUI実施）で新規バグを2件発見・修正。(1) glm-ocr がページ全体のOCR結果を一度出力した後、```markdown フェンスに包んで内容をもう一度出力し直す癖があり（表クロップの再OCRで確認されていた同種の癖がページ全体でも発生）、フェンス内の再出力は誤字混入・フェンスが閉じないまま出力終了するなど後半にいくほど劣化する「劣化する再生成」であることを実データ5ページ（Page3/5/7/12/15）で確認。フェンス内容がそれ以前の本文と5割以上一致する場合はフェンス以降を切り捨てる `truncate_duplicate_reemission` を追加。(2) 図表の座標配置（元のページ内の該当箇所に挿入）は現構成では非対応で、`detect_figures.py` が検出した bbox が Rust 側に渡っていないため常にページ末に一括追記される既存の制約と判明（`docs/tasks.md` の「案C VLMでbbox検出→却下」と同根、Ollama移行時の設計上の制約であり今回の変更が原因ではない）。LaTeX出力の不安定さは `sanitize_math_delimiters` 自体は正しく動作しており、glm-ocr が生成するLaTeX構文自体の精度揺らぎ（括弧の閉じ忘れ等）が原因と判断し、対応は見送り
  → 検証: `cargo test --lib`（9 passed, 1 ignored、フェンス重複を模した回帰テスト2件を含む）。実機再検証: `1周間SPI_模擬4.pdf` p3・p5（実際にフェンス重複が発生していた箇所）を `reocr_pdf_manual` で2回再実行し、いずれもフェンス重複なし・内容欠落なしを確認
- [x] フェンス重複修正後の実機再検証（`1周間SPI_模擬4.pdf` p3-15、ユーザーGUI実施）でフェンス重複は解消したが、`\textcircled{N}`（丸数字①②③...）がレンダリングされない問題を確認。原因は、原資料の丸数字を glm-ocr が LaTeX コマンドとして符号化するが、KaTeX 等の軽量 Markdown レンダラは `\textcircled` に非対応で描画されないこと。`\textcircled{N}`（1〜20）を対応する Unicode 丸数字文字（①〜⑳）に変換する `convert_circled_numbers` を追加し、数式レンダリングに頼らず表示できるようにした。一方、`\underbrace{...}` の閉じ括弧忘れ等の壊れたLaTeXや、デリミタなしで生の `\frac{}{}` が出力されるケースは glm-ocr 自体の生成揺らぎであり、構文修復は個別対応の域を超えるため見送り
  → 検証: `cargo test --lib`（11 passed, 1 ignored、丸数字変換の新規テスト2件を含む）で確認済み。実機での `\textcircled` 変換の再検証は未実施
- [ ] 図表の座標配置（bbox を Rust 側に渡し、OCRテキスト内の該当位置に画像リンクを挿入する）は未対応のまま。対応する場合は `detect_figures.py` の出力に bbox 座標を追加し、`figure_extraction::extract_figures` の戻り値型を拡張した上で、OCRテキスト内の挿入位置をどう決定するか（ページ内での相対位置とテキスト行の対応付け）を別途設計する必要がある
  → 検証: 対応するかどうかも含め方針未確定
