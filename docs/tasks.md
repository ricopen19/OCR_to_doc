...
- [x] ハルシネーション対策のギャップを解消: `truncate_runaway_repetition`（ocr/pipeline.rs）が直前1行との類似度しか見ておらず、数行単位のブロック（表の一部）が周期的に微妙な文字化けを伴いながら繰り返される暴走パターンを検知できていなかった問題を修正。ブロック長 1〜8行の全パターンを試し、各ブロック長について境界オフセットをずらした全開始位置で「直前ブロックとの類似判定チェーン」を評価する `detect_block_repeat_cut` を新設。単一行判定（ブロック長1）は既存ロジックと完全に同じ結果になるよう設計し、複数のブロック長で検知した場合は最も早い切り捨て位置を採用する
  → 検証: `cargo test --lib`（29 passed, 2 ignored）で確認済み。既存の単一行反復テスト3件が回帰なく通過することに加え、`reocr_pdf_manual` の実機観測（4行ブロックが4〜5回重複）を模した新規回帰テスト（4行ブロック反復を検知して切り捨てる／表の数行一致程度では誤って切り捨てない）2件を追加し通過を確認。実機PDFでの再検証（重複ブロックが出力に残らないこと）は未実施
...
- [ ] ログで得たスコア分布（正常1.00 vs 破綻0.90〜0.91）を踏まえ、実際に「閾値未満の表だけ再OCRする」という本番の分岐ロジックへの切り替え。閾値をどこに置くか（サンプル数が少ないため、より多くの実データでの分布確認が必要）と、閾値未満と判定された履歴を後から目視検証できるログの整備が前提
  → 検証: 閾値を決定し分岐を実装した上で、実際に23箇所前後の表を含むPDFで「再OCR対象が本当に減ったか」「見逃し（本当は壊れているのに再OCRされなかった表）がないか」を目視確認する
