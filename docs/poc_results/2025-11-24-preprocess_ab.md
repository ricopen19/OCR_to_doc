...
注: 本ドキュメントは **当時の検証ログ** です。現状の確定仕様・既定値・CLI は `docs/spec.md` / `docs/cli_commands.md` を参照してください（フラグ名が変更されている場合があります）。
...
- （当時のフラグ名）`--keep-page-images` で元画像を保存（現状はデフォルトで保存、不要なら `--drop-page-images`）。
