...
- ヘルプ
  - このアプリの使い方（クイックスタート / 注意点 / 問い合わせ先）
  - バージョン情報（アプリ情報 / クレジット / 注意事項）
- OCR 実行
  ...
  - デフォルト保存: 実行画面の「デフォルトに保存」で `formats / excelMode / mode / pdfDpi` を `configs/settings.json` に保存
  - 実行: スタートのみ（キャンセルは未実装）
...
- 設定
  ...
  - ウィンドウサイズ、出力先、画像PDF化/図表抽出、Excelメタシート、GPU、チャンクサイズ、休憩設定
...
- サイドバー: ホーム / 実行 / 結果 / 設定 / ヘルプ（使い方 / バージョン情報）
- フッター: 現状なし
